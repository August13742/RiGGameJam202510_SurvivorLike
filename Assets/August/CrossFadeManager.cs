...
namespace AugustsUtility.UX
{
    public class CrossfadeManager : MonoBehaviour
    {
        ...
        /// </summary>
        public ITween FadeToBlack(float duration = 1f)
        {
            // Don't allow clicks during the fade
            canvasGroup.blocksRaycasts = true;
            return canvasGroup.TweenAlpha(1f, duration);
        }

        /// <summary>
        /// Fades the screen FROM black.
        /// </summary>
        public ITween FadeFromBlack(float duration = 1f)
        {
            // Return a tween so the caller can wait for completion
            return canvasGroup.TweenAlpha(0f, duration, onComplete: (() =>
            {
