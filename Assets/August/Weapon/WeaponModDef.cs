...
namespace Survivor.Weapon
{
    public abstract class WeaponModDef : ScriptableObject
    {
        ...
        public virtual void OnHit(IWeapon weapon, float dmg, Vector2 pos, bool crit) { }
        ...
        {
            if (weapon is IModTarget wt) return wt.GetContext();
            return null;
        }
        // Helper to get owner transform
        protected Transform GetOwner(IWeapon weapon)
        {
            return (weapon is IModTarget wt) ? wt.GetContext()?.Owner : null;
        }
    }
}