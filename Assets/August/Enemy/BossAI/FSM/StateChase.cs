...
namespace Survivor.Enemy.FSM
{
	public class StateChase : IState
	{
		...
		public Type Tick(float deltaTime)
		{
			...
			// Movement policy
			if (band == RangeBand.OffBand || band == RangeBand.Pocket) // Off band OR Pocket but no ranged play Å® nudge toward melee
			{
			...
			else // Melee band but dry Å® wander instead of face-tanking nothing
			{
